...
void ANOVA(double *objs,int **LL, int nsample, int nparam, float *F, float *R2)
{
	int i,j,k,l;
...
void Bt_ANOVA(int nsample, int nparam, int np, int nrspl, int **LL, double *stobj, float *FCI)
{
	int i,j,k;
	double *obj;
	int rspl;
...
int main(int argc, char* argv[]) 
{
	...
	double* obj=DoubleVector(nsample); //objective (model response) vector
	...
	int i, j, k;
        double Anovapara[nparam];

	//initialize random seed
	srand((unsigned)time( NULL )); //for time-based seeding, use 

        //read parameter lower and upper bounds
        FILE*parameterfile;
        char string[1000];
        parameterfile=fopen("parameter-range-test","r");//"parameter" is the parameter file,
                                                   //it must in the same file with anova.c(executable files)
        if (parameterfile==NULL)
        {
                printf("can not open parameter-range file.\n");
                exit(1);
        }
        fgets(string,1000,parameterfile);
        for (i=0; i<nparam; i++)
        {
                  fscanf(parameterfile,"%lf",&lb[i]);
                  fscanf(parameterfile,"%lf",&ub[i]);
        }
	...
	IFFD(nrep,nparam,ncol,nzero,lb,ub,par,LL);
        ...
        //parameter Level matrix "LL" is writed in LL_file.
        FILE*LL_file;
        LL_file=fopen("LL_file","w");
        if (LL_file==NULL)
        {
        ...
        for (i=0; i<nsample; i++) 
        {
                  for (j=0; j<nparam; j++)
                  {
                            if (j==nparam-1)
                            {
                                    fprintf(LL_file,"%d\n",LL[i][j]);//if j=nparam-1, newline start. 
                            }   
                             else   fprintf(LL_file,"%d\t",LL[i][j]);
                  }
        }
        printf("the end of writing LL_file\n"); 
        fclose(LL_file);

        //the samples array "par" is writed in parfile.
        FILE*samplefile;
        samplefile=fopen("par","w");
        if (samplefile==NULL)
        {
        ...
        for (i=0; i<nsample; i++) 
        {
                  for (j=0; j<nparam; j++)
                  {
                            if (j==nparam-1)
                            {
                                    fprintf(samplefile,"%lf\n",par[i][j]);//if j=nparam-1, newline start.
                            }   
                             else   fprintf(samplefile,"%lf\t",par[i][j]);
                  }
        }
        printf("the end of writing array par\n"); 
        fclose(samplefile);
